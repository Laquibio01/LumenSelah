...
class _LessonsPathScreenState extends State<LessonsPathScreen> {
  ...
  Widget _buildDynamicNode(int id, ColorScheme colorScheme, Color textColor, ThemeData theme) {
    bool isComp = id < _unlockedLesson;
    bool isCurr = id == _unlockedLesson;
    String title = lessonsDatabase[id]?.title ?? 'Lección $id';
    return _buildNode(title, isCompleted: isComp, isCurrent: isCurr, colorScheme: colorScheme, textColor: textColor, theme: theme);
  }
    ...
    return Column(
      ...
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            _buildDynamicNode(1, colorScheme, textColor, theme),
            Container(width: 50, height: 4, color: isCompleted(1) || isCurrent(1) ? lineColor : Colors.grey.withValues(alpha: 0.2)),
            _buildDynamicNode(2, colorScheme, textColor, theme),
          ],
        ),
        Transform.translate(
          offset: const Offset(70, 0),
          child: Container(width: 4, height: 40, color: isCompleted(2) ? lineColor : Colors.grey.withValues(alpha: 0.2)),
        ),
        Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            _buildDynamicNode(4, colorScheme, textColor, theme),
            Container(width: 50, height: 4, color: isCompleted(3) || isCurrent(3) ? lineColor : Colors.grey.withValues(alpha: 0.2)),
            _buildDynamicNode(3, colorScheme, textColor, theme),
          ],
        ),
        Transform.translate(
          offset: const Offset(-70, 0),
          child: Container(width: 4, height: 40, color: isCompleted(4) ? lineColor : Colors.grey.withValues(alpha: 0.2)),
        ),
        Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            _buildDynamicNode(5, colorScheme, textColor, theme),
            Container(width: 50, height: 4, color: isCompleted(5) || isCurrent(5) ? lineColor : Colors.grey.withValues(alpha: 0.2)),
            _buildDynamicNode(6, colorScheme, textColor, theme),
          ],
        ),
      ...
      decor = BoxDecoration(
        gradient: LinearGradient(
          colors: theme.brightness == Brightness.dark
              ? [colorScheme.primary.withValues(alpha: 0.85), colorScheme.surface]
              : [colorScheme.primary.withValues(alpha: 0.85), colorScheme.primary],
